module.exports = (function ( f ) {
    ...
    $.perform = function ( ctx, fnName ) {
        var slice = Array.prototype.slice

        if ( !ctx || !f.is.function( ctx[fnName] )) {
            throw new TypeError(`ctx[fnName] must be a function, ${ctx}, ${fnName}`)
        }


        return ctx[fnName].apply(ctx, slice.call( arguments, 2 ))
    }
