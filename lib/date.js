...
module.exports = function (f) {
  ...
  $.fromString = function str2date(str) {
    var exec = /(\d{2,4})-(\d{1,2})-(\d{1,2})\s(\d{1,2}):(\d{1,2}):(\d{1,2})/.exec(str);
    var res = new Date(0);
    if (exec && exec[1]) {
  ...
  $.toString = function (date) {
    ...
    var replace = function replace(v) {
      switch (v) {
        case "YYYY":
    ...
    return format.replace(/Y{4}|Y{2}|M{1,2}|D{1,2}|H{1,2}|m{1,2}|s{1,2}|W|w/g, function ($0) {
      return replace($0);
    });
  };
  ...
  $.timeMS2HHmmss = function (timeMS) {
    if (timeMS === null || timeMS === undefined) return 'Info err.';

    var timeS = parseInt(timeMS / 1000);
    var hour = parseInt(timeS / 3600);
    var secondsInHour = hour * 3600;
