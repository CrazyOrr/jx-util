module.exports = (function ( f ) {
  var $ = f.array = {}
	...
	$.head = $.first = arr => arr[0]
	$.tail = $.last = arr => arr[arr.length - 1]
  ...
  // 清空
	 ...
	 */
	$.adjust = f.function.curryN(3, (list, idx, fn) => {
	 ...
	 */
	$.allPass = f.function.curryN(2, (arr = [], fn) => {
	 ...
	 */
	$.anyPass = f.function.curryN(2, (arr = [], fn) => { // 操作符 保持在 操作对象后面
		for (let i = 0; i < arr.length; i++) {
			if (fn(arr[i], i, arr)) {
				return true
			}
	...
	$.append = f.function.curryN(2, (list, ele) => (
		concat(list, [ele])
	))
	...
	$.slice = f.function.curryN(3, (list = [], fromIndex, toIndex) => list.slice(fromIndex, toIndex))
	...
	$.drop = f.function.curryN(2, (n, xs) => (
		f.array.slice(Math.max(0, n), Infinity, xs)
	));
})